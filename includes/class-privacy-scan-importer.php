...
class Privacy_Scan_Importer {
	...
	public static function import_report( array $report ) {
		...
		// WordPress-side SMTP / Gravity SMTP connectors (invisible to Playwright network).
		if ( class_exists( __NAMESPACE__ . '\\Cookie_Scanner' ) ) {
			$scanner     = Cookie_Scanner::instance();
			$tx_findings = $scanner->scan_transactional_email_public();
			foreach ( $tx_findings as $finding ) {
				if ( ! is_array( $finding ) ) {
					continue;
				}
				$results[] = $finding;
				if ( ! empty( $finding['service'] ) ) {
					$detected[ sanitize_key( $finding['service'] ) ] = true;
				}
			}
		}
		...
		$scanned_page_urls = self::sanitize_scanned_page_urls( isset( $report['pages'] ) ? $report['pages'] : array() );
		$reverify_hint     = self::build_reverify_hint( $findings, $consent_leaks, $scanned_page_urls );
		$tx_summary        = array(
			'detected'  => false,
			'providers' => array(),
		);
		if ( class_exists( __NAMESPACE__ . '\\Cookie_Scanner' ) ) {
			$tx_summary = Cookie_Scanner::instance()->summarize_transactional_from_results_public( $results );
		}
		...
		$payload = array(
			...
			'scanned_urls'       => count( $scanned_page_urls ),
			'scanned_page_urls'  => $scanned_page_urls,
			'reverify_hint'      => $reverify_hint,
			'transactional_email'=> $tx_summary,
			'storage'            => $storage,
	...
	public static function apply_safe_updates( array $imported, array $previous = array() ) {
		...
		if ( class_exists( __NAMESPACE__ . '\\Cookie_Scanner' ) ) {
			...
			foreach ( $scanner->scan_transactional_email_public() as $finding ) {
				if ( ! empty( $finding['service'] ) ) {
					$keys[] = sanitize_key( $finding['service'] );
				}
			}
	...
	public static function resolve_service_key( $provider, $url = '' ) {
		...
		$aliases = array(
			...
			'mailtrap'                 => array( 'mailtrap', 'smtp.mailtrap.io', 'send.api.mailtrap.io' ),
	...
	/**
	 * Sanitize Playwright report page URL list.
	 *
	 * @param mixed $pages Report pages[].
	 * @return string[]
	 */
	private static function sanitize_scanned_page_urls( $pages ) {
		if ( ! is_array( $pages ) ) {
			return array();
		}
		$out = array();
		foreach ( array_slice( $pages, 0, 100 ) as $page ) {
			$url = esc_url_raw( is_string( $page ) ? $page : '' );
			if ( $url ) {
				$out[] = $url;
			}
		}
	...
	/**
	 * Prefer homepage, then remaining scanned pages, capped for re-verify jobs.
	 *
	 * @param string[] $page_urls Page URLs.
	 * @param int      $cap       Max pages.
	 * @return string[]
	 */
	private static function prioritize_reverify_urls( array $page_urls, $cap ) {
		$cap  = max( 1, (int) $cap );
		$home = untrailingslashit( home_url( '/' ) );
		$home_slash = trailingslashit( $home );
		$ordered = array();
		$seen    = array();

		$push = static function ( $url ) use ( &$ordered, &$seen, $cap ) {
			$url = esc_url_raw( (string) $url );
			if ( ! $url || isset( $seen[ $url ] ) || count( $ordered ) >= $cap ) {
				return;
			}
			$seen[ $url ] = true;
			$ordered[]    = $url;
	...
	/**
	 * Build a fast re-verify hint (Light sessions + scanned pages; Standard if GPC/DNS fails).
	 *
	 * @param array    $findings      Sanitized findings.
	 * @param array    $consent_leaks Sanitized leaks.
	 * @param string[] $page_urls     Scanned page URLs.
	 * @return array{depth:string,page_urls:string[],fail_codes:string[],has_fails:bool}
	 */
	public static function build_reverify_hint( array $findings, array $consent_leaks, array $page_urls ) {
		$fail_findings = array(
		...
		foreach ( $findings as $row ) {
			if ( ! is_array( $row ) || empty( $row['finding'] ) ) {
				continue;
			}
			$code = sanitize_key( (string) $row['finding'] );
			if ( in_array( $code, $fail_findings, true ) ) {
				$codes[ $code ] = true;
			}
		}
		$needs_standard = isset( $codes['still_loaded_after_gpc'] ) || isset( $codes['still_loaded_after_dns'] );
		$mapped           = self::depth_to_scanner_options( $needs_standard ? 'standard' : 'quick' );
		$cap              = (int) $mapped['maxPages'];
		if ( $cap < 1 ) {
			$cap = $needs_standard ? 10 : 8;
		}
		// Keep re-verify page lists tight even when Standard sessions are required.
		$cap = min( $cap, $needs_standard ? 10 : 8 );

		return array(
			'depth'      => $needs_standard ? 'standard' : 'quick',
			'page_urls'  => self::prioritize_reverify_urls( $page_urls, $cap ),
			'fail_codes' => array_keys( $codes ),
			'has_fails'  => ! empty( $codes ) || ! empty( $consent_leaks ),
		);
	}

	/**
	 * Re-verify hint from the last imported Playwright scan (rebuilds if missing).
	 *
	 * @return array{depth:string,page_urls:string[],fail_codes:string[],has_fails:bool}
	 */
	public static function reverify_hint_from_last_scan() {
		$scan = Cookie_Scanner::instance()->get_last_scan();
		if ( ! is_array( $scan ) ) {
			return array(
				'depth'      => 'quick',
				'page_urls'  => self::prioritize_reverify_urls( array(), 8 ),
				'fail_codes' => array(),
				'has_fails'  => false,
			);
		}
		if ( ! empty( $scan['reverify_hint'] ) && is_array( $scan['reverify_hint'] ) ) {
			$hint = $scan['reverify_hint'];
			$depth = ! empty( $hint['depth'] ) && 'standard' === $hint['depth'] ? 'standard' : 'quick';
			$pages = ! empty( $hint['page_urls'] ) && is_array( $hint['page_urls'] ) ? $hint['page_urls'] : array();
			$codes = ! empty( $hint['fail_codes'] ) && is_array( $hint['fail_codes'] ) ? array_map( 'sanitize_key', $hint['fail_codes'] ) : array();
			return array(
				'depth'      => $depth,
				'page_urls'  => self::prioritize_reverify_urls( $pages, 'standard' === $depth ? 10 : 8 ),
				'fail_codes' => array_values( array_filter( $codes ) ),
				'has_fails'  => ! empty( $hint['has_fails'] ) || ! empty( $codes ),
			);
		}
		$pages = ! empty( $scan['scanned_page_urls'] ) && is_array( $scan['scanned_page_urls'] )
			? $scan['scanned_page_urls']
			: array();
		return self::build_reverify_hint(
			! empty( $scan['findings'] ) && is_array( $scan['findings'] ) ? $scan['findings'] : array(),
			! empty( $scan['consent_leaks'] ) && is_array( $scan['consent_leaks'] ) ? $scan['consent_leaks'] : array(),
			$pages
		);
	}
