		...
		<main class="ucpf-wizard__panel">
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" id="ucpf-wizard-form">
				...
				<?php elseif ( 3 === $step ) : ?>
					...
					<h3><?php esc_html_e( 'Site profile', 'universal-consent-privacy-framework' ); ?></h3>
					<p><?php esc_html_e( 'Shapes scan page defaults (and optional logged-in pass). Enforcement stays consent-based for optional trackers on every profile.', 'universal-consent-privacy-framework' ); ?></p>
					<fieldset class="ucpf-wizard__fieldset">
						<?php foreach ( $profiles as $profile_key => $profile_meta ) : ?>
							<label>
								<input type="radio" name="site_profile" value="<?php echo esc_attr( $profile_key ); ?>" <?php checked( $site_profile, $profile_key ); ?>
									<?php disabled( \UCPF\Site_Profiles::WOOCOMMERCE === $profile_key && ! $woo_active ); ?> />
								<strong><?php echo esc_html( $profile_meta['label'] ); ?></strong>
								<span class="description" style="display:block;margin:0.15rem 0 0.65rem 1.6rem;"><?php echo esc_html( $profile_meta['description'] ); ?></span>
							</label>
						<?php endforeach; ?>
					</fieldset>
					<?php if ( ! $woo_active ) : ?>
						<p class="description"><?php esc_html_e( 'WooCommerce is not active on this site — the store profile is disabled until WooCommerce is installed.', 'universal-consent-privacy-framework' ); ?></p>
					<?php endif; ?>
