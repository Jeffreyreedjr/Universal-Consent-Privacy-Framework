...
export async function discoverSitePaths(siteUrl, opts = {}) {
  ...
  const add = (raw, score = 6) => {
    try {
      ...
      let p = u.pathname || '/';
      if (!p.startsWith('/')) p = `/${p}`;
      // Drop assets / feeds / downloads (PDF etc. break Playwright goto).
      if (/\.(css|js|map|png|jpe?g|gif|webp|svg|ico|woff2?|ttf|eot|pdf|docx?|xlsx?|pptx?|zip|rar|7z|gz|mp[34]|xml|json)$/i.test(p)) return;
